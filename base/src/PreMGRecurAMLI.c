...
// 60  220  85
#if FASP_GSRB
INT  nx_rb   = 60 ;   /**< Red Black GS Smoother Nx */
INT  ny_rb   = 220 ;   /**< Red Black GS Smoother Ny */
INT  nz_rb   = 85 ;   /**< Red Black GS Smoother Nz */
                        ...
                        INT         num_levels)
{
    ...
    if ( level < num_levels-1 ) {
        ...
        if ( level < mgl[level].ILU_levels ) {
            ...
            fasp_smoother_dcsr_gs_multicolor (&mgl[level].x, &mgl[level].A, &mgl[level].b, param->presmooth_iter,1);
#else            
#if FASP_GSRB
            if ( (level==0) && (nx_rb>1) )
                fasp_smoother_dcsr_gs_rb3d(e0, A0, b0,
...
#if MULTI_COLOR_ORDER
	    fasp_smoother_dcsr_gs_multicolor (&mgl[level].x, &mgl[level].A, &mgl[level].b, param->postsmooth_iter,-1);
#else
#if FASP_GSRB
            if ( (level==0) && (nx_rb>1) )
                fasp_smoother_dcsr_gs_rb3d(e0, A0, b0,
                                           param->postsmooth_iter, -1, IMAP, MAXIMAP,
                                           nx_rb, ny_rb, nz_rb);
            else
                fasp_dcsr_postsmoothing(smoother,A0,b0,e0,param->postsmooth_iter,
                                    0,m0-1,-1,relax,ndeg,smooth_order,ordering);
