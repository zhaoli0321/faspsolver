...
#if 0       // print detials information
            for(i=0; i < mgl[lvl].A.color; i++ ){
                // for(j=A -> IC[i]; j < A-> IC[i+1];j++)
                //     printf("color %d  ICMAP[%d] = %d \n", i,j,A-> ICMAP[j]);
                printf( "mgl[%3d], c = %d, A.color = %d, A.row= %d %d\n", lvl, i, mgl[lvl].A.color, mgl[lvl].A.row, mgl[lvl].A.IC[i+1] - mgl[lvl].A.IC[i] );
            }
#endif
	} // end if
    }//end for
